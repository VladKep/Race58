...
function startGame() {
	...
	opsBlock.style.display = "block";
	audio7 = new Audio('audio/236.mp3');
	audio7.play();
...
document.onkeydown = function (event) {
	if ((event.keyCode == 37&&gamer.offsetLeft > 340  )) {
		gamer.style.left = gamer.offsetLeft - 80 + "px";
	} if (( event.keyCode == 39&&gamer.offsetLeft < 1100 )) {
		gamer.style.left = gamer.offsetLeft + 80 + "px";
	}
}
// Создание врага
	...
	let timerId = setInterval(function () {
		if (score >= 5) {
		    ...
		    if (score >= 200) {
				...
				audio26 = new Audio('audio/369.mp3');
				audio26.play();
...
function load() {
	audioPlayer.pause();
	audio25 = new Audio('audio/74.mp3');
	audio25.play();
}
function check() {
